...
public class MainInterface extends javax.swing.JFrame {
    ...
    private String hostname, password, recipient;
            ...
            public void actionPerformed(ActionEvent ae) {
                new Thread(){
                    ...
                    public void run() {
                        ...
                        try{sleep(100); //per aspettare che i thread concorrenti chiudano la connessione prima che questo lo apra
    ...
    public void disconnesso(){
        ...
        hostname= MyClient.KW_C;
        recipient= MyClient.KW_L;
        reloadClients();
        reloadChatPanel();
    ...
    public void addMessage(String[] message){
        ...
        time= message[2];
        text= message[3];
        if(isChatMessageValido(writer, recipient)){
    ...
    private void reloadChatPanel(){
        if(allChatsCheckBox.isSelected()) chatLabel.setText("All Chats");
        else if(recipient.equals(MyClient.KW_G)) chatLabel.setText("Global Chat");
        else if(recipient.equals(MyClient.KW_L)) chatLabel.setText("Local Chat");
        else chatLabel.setText("Private Chat (" + recipient + ")");
        messageLabel.setText("Send Message (destinazione " + recipient + ")");
        reloadMessages();
    }
    
    private boolean isChatMessageValido(String writer, String recipient){
        boolean isValido= false;
        if(allChatsCheckBox.isSelected()) isValido= true;
        else if(isKeyWord(this.recipient) && writer.equals(this.recipient) || recipient.equals(this.recipient)) isValido= true;
        else if(writer.equals(this.recipient) && recipient.equals(hostname) || writer.equals(hostname) && recipient.equals(this.recipient)) isValido= true;
        return isValido;
    }
    
    private boolean isHostnameValido(String hostname){
        if(hostname.trim().length() < 4) return false;
    ...
    private void sendButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_sendButtonActionPerformed
        ...
        if(message.trim().length() > 0){
            if(recipient.equals(MyClient.KW_L)) client.addLocalMessage(MyClient.KW_C, message);
            else client.sendMessage(recipient, message);
        }
    ...
    private void nameTextFieldCaretUpdate(javax.swing.event.CaretEvent evt) {//GEN-FIRST:event_nameTextFieldCaretUpdate
        ...
        nameLogLabel.setText(" ");
        if(isHostnameValido(nameTextField.getText())) confermaNameButton.setEnabled(true);
        else nameLogLabel.setText("*Nome non valido ");
    }//GEN-LAST:event_nameTextFieldCaretUpdate
    private void nameTextFieldKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_nameTextFieldKeyPressed
        if(evt.getKeyCode() == java.awt.event.KeyEvent.VK_ENTER) confermaNameButtonActionPerformed(null);
    }//GEN-LAST:event_nameTextFieldKeyPressed
    private void PassPasswordFieldKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_PassPasswordFieldKeyPressed
        if(evt.getKeyCode() == java.awt.event.KeyEvent.VK_ENTER) confermaPassButtonActionPerformed(null);
    ...
    private void selectButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_selectButtonActionPerformed
        ...
        if(this.recipient.equals(recipient)) this.recipient= MyClient.KW_G;
    ...
    private void clientsListValueChanged(javax.swing.event.ListSelectionEvent evt) {//GEN-FIRST:event_clientsListValueChanged
        ...
        if(index != -1) selectButton.setEnabled(true);
